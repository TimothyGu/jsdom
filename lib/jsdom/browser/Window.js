...
let bootstrapingDefaultWindow = true;
const defaultCore = (() => {
  const tmpWindow = Object.create(Window.prototype);
  const binding = contextifyWindow(tmpWindow);
  binding.defaultPrivateData.window = tmpWindow;
  tmpWindow._core = binding.exports;
  Window.call(tmpWindow, {});
  return binding.exports;
})();
bootstrapingDefaultWindow = false;
...
require("../living").extras.Window = Window;
...
function Window(options) {
  ...
  let dom;
  if (bootstrapingDefaultWindow) {
    dom = this._core;
  } else {
    const binding = contextifyWindow(this);
    binding.defaultPrivateData.window = this;
    dom = binding.exports;
    this._core = dom;
  }

  dom.EventTarget.setup(this, [], { window });
  if (bootstrapingDefaultWindow) {
    GlobalEventHandlersImpl.prototype._initGlobalEvents.call(this);
  } else {
    this._initGlobalEvents();
  }
  idlUtils.implForWrapper(this)[idlUtils.wrapperSymbol] = this._globalProxy;

  let timers = Object.create(null);
  let animationFrameCallbacks = Object.create(null);

  // List options explicitly to be clear which are passed through
  this._document = dom.Document.create([], {
    options: {
  ...
  const external = dom.External.create();
  const navigator = dom.Navigator.create([], { userAgent: options.userAgent });
...
Object.setPrototypeOf(Window, defaultCore.EventTarget.interface);
Object.setPrototypeOf(Window.prototype, defaultCore.EventTarget.interface.prototype);

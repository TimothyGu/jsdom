...
exports.contextifyWindow = windowImpl => {
  const window = windowImpl._wrapper;
  if (vm.isContext(window)) {
    return;
  }

  vm.createContext(window);
  windowImpl._globalProxy = vm.runInContext("this", window);

  // XXX
  for (const key of Object.getOwnPropertyNames(windowImpl._globalProxy)) {
    if (Object.getOwnPropertyDescriptor(window, key) === undefined) {
      Object.defineProperty(window, key, Object.getOwnPropertyDescriptor(windowImpl._globalProxy, key));
    }
  }
};

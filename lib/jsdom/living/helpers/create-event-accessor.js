...
exports.appendHandler = function appendHandler(el, eventName) {
  el.addEventListener(eventName, event => {
    ...
    // el can be a Window, and until Window is migrated to webidl2js _window is only defined on a Window's impl object.
    // Therefore, try to get the impl.
    const specialError = idlUtils.tryImplForWrapper(el)._window._core.ErrorEvent.isImpl(event) &&
      event.type === "error" &&

...
class HTMLInputElementImpl extends HTMLElementImpl {
  ...
  _activationBehavior() {
    ...
    if (this.type === "checkbox") {
      const inputEvent = this._window._core.Event.createImpl(["input", { bubbles: true, cancelable: true }], {});
      this.dispatchEvent(inputEvent);

      const changeEvent = this._window._core.Event.createImpl(["change", { bubbles: true, cancelable: true }], {});
  ...
  get files() {
    if (this.type === "file") {
      this[filesSymbol] = this[filesSymbol] || this._window._core.FileList.createImpl();
    } else {

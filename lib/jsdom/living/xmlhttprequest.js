...
// TODO: convert this to Web IDL

module.exports = function createXMLHttpRequest(window) {
  const { Event, ProgressEvent } = window;

  class XMLHttpRequest extends window._core.XMLHttpRequestEventTarget.interface {
    constructor() { // eslint-disable-line constructor-super
      const theThis = Object.create(new.target.prototype);
      window._core.XMLHttpRequestEventTarget.setup(theThis);
      theThis.upload = window._core.XMLHttpRequestUpload.create();
    ...
    get responseXML() {
      ...
      const res = window._core.Document.create([], { options: {
        url: flag.uri,
    ...
    send(body) {
      ...
      try {
        ...
        if (body !== null) {
          ...
          if (window._core.Document.isImpl(body)) {
            encoding = "UTF-8";
  ...
  function coerceBodyArg(body) {
    ...
    if (impl) {
      ...
      if (window._core.Blob.isImpl(impl) || window._core.FormData.isImpl(impl) || window._core.Document.isImpl(impl)) {
        return impl;
      }
    }
  ...
  function extractBody(bodyInit) {
    ...
    if (window._core.Blob.isImpl(bodyInit)) {
      return {
    ...
    } else if (window._core.FormData.isImpl(bodyInit)) {
      const formData = [];
      for (const entry of bodyInit._entries) {
        let val;
        if (window._core.Blob.isImpl(entry.value)) {

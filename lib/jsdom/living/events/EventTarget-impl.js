...
class EventTargetImpl extends WebIDLInterfaceImpl {
  ...
  dispatchEvent(eventImpl) {
    const { Event } = this._window._core;
  ...
  _dispatch(eventImpl, targetOverride) {
    ...
    for (let i = eventPath.length - 1; i >= 0; --i) {
      ...
      invokeEventListeners(Event, eventListeners, object, eventImpl);
    }
    ...
    if (!eventImpl._stopPropagationFlag) {
      if (this._eventListeners[eventImpl.type]) {
        ...
        invokeEventListeners(Event, eventListeners, eventImpl.target, eventImpl);
      }
    }

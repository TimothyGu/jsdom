...
class EventTargetImpl extends WebIDLInterfaceImpl {
  ...
  dispatchEvent(eventImpl) {
    ...
    if (eventImpl.eventPhase !== this._window.Event.NONE) {
  ...
  _dispatch(eventImpl, targetOverride) {
    ...
    eventImpl.eventPhase = this._window.Event.CAPTURING_PHASE;
    for (let i = eventPath.length - 1; i >= 0; --i) {
      ...
      invokeEventListeners(this._window.Event, eventListeners, object, eventImpl);
    }

    eventImpl.eventPhase = this._window.Event.AT_TARGET;

    if (!eventImpl._stopPropagationFlag) {
      if (this._eventListeners[eventImpl.type]) {
        const eventListeners = this._eventListeners[eventImpl.type];
        invokeEventListeners(this._window.Event, eventListeners, eventImpl.target, eventImpl);
      }
    }

    if (eventImpl.bubbles) {
      eventImpl.eventPhase = this._window.Event.BUBBLING_PHASE;
    ...
    eventImpl.eventPhase = this._window.Event.NONE;
...
function invokeEventListeners(Event, listeners, target, eventImpl) {

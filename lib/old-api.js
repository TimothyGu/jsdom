...
const createWindow = require("./jsdom/browser/Window");
...
exports.jsdom = function (html, options) {
  ...
  if (options.created) {
    options.created(null, window._globalProxy);
  }
  ...
  if (window._document.close && !options.deferClose) {
...
exports.jQueryify = exports.jsdom.jQueryify = function (window, jqueryUrl, callback) {
  ...
  const document = windowImpl._document;
  const implImpl = document.implementation;
  ...
  documentFeatures.contextifyWindow(windowImpl._wrapper);
  ...
  scriptEl.onload = scriptEl.onerror = () => {
    ...
    windowImpl._runScripts = oldRunScripts;
...
exports.evalVMScript = (window, script) => {
  return script.runInContext(idlUtils.implForWrapper(window)._wrapper);
};
